...
model = torch.hub.load('ultralytics/yolov5', 'custom', path=r'C:\Users\carla\Desktop\bambooImageProcessing\yolov5\runs\train\exp5\weights\best.pt')  # Adjust the path to your model
model = model.to('cuda')  # Ensure the model runs on GPU
...
def use_this_photo():
    ...
    if selected_image_path:
        ...
        results = model(selected_image_path)  # Run inference on the selected image
        ...
        use_photo_button.pack_forget()  # Hide the "Use this photo" button after using the image
